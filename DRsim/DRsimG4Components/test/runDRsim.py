...
pgun = MomentumRangeParticleGun(
    ...
    MomentumMin=20.0 * units.GeV,  # GeV
    MomentumMax=20.0 * units.GeV,  # GeV
    ThetaMin=1.5335,  # rad
    ThetaMax=1.5335,  # rad
    PhiMin=0.01745,  # rad
    PhiMax=0.01745,  # rad
...
from Configurables import (
    SimG4Svc,
    SimG4FastSimPhysicsList,
    SimG4FastSimOpFiberRegion,
    SimG4OpticalPhysicsList,
)

regionTool = SimG4FastSimOpFiberRegion("fastfiber")
opticalPhysicsTool = SimG4OpticalPhysicsList(
    "opticalPhysics", fullphysics="SimG4FtfpBert"
)
...
saveDRcaloTool = SimG4SaveDRcaloHits(
    "saveDRcaloTool", readoutNames=["DRcaloSiPMreadout"]
)
