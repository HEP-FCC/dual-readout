...
FastFiberData::FastFiberData(G4int id, G4double en, G4double globTime, G4double path, G4ThreeVector pos,
                             G4ThreeVector mom, G4ThreeVector pol, G4int status) {
  trackID = id;
  kineticEnergy = en;
  globalTime = globTime;
  pathLength = path;
...
G4bool FastSimModelOpFiber::ModelTrigger(const G4FastTrack& fasttrack) {
  ...
  // make sure that the track does not get absorbed after transportation, as number of interaction length left is reset
  ...
  if (fTransported) { // track is already transported and did pass NILL check, nothing to do
    if (mFiberAxis.dot(track->GetMomentumDirection()) * mFiberAxis.dot(mDataCurrent.momentumDirection) <
...
G4bool FastSimModelOpFiber::checkTotalInternalReflection(const G4Track* track) {
  ...
  // some cases have a status StepTooSmall when the reflection happens between the boundary of cladding & outer volume
  ...
  if (theStatus == G4OpBoundaryProcessStatus::TotalInternalReflection ||
    ...
    if (theStatus != G4OpBoundaryProcessStatus::TotalInternalReflection) { // skip StepTooSmall if the track already has
...
void FastSimModelOpFiber::DefineCommands() {
  ...
  G4GenericMessenger::Command& safetyCmd =
      mMessenger->DeclareProperty("safety", fSafety, "min number of total internal reflection");
