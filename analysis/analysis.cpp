...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int iEvt = 0; iEvt < entries; iEvt++) {
    ...
    for (unsigned int idx = 0; idx < caloHits->size(); idx++) {
      ...
      for (unsigned int bin = 0; bin < timeStruct.adcCounts_size(); bin++) {
        ...
        tT->Fill(timeStruct.getTime() + (static_cast<float>(bin) + 0.5) * timeStruct.getInterval(),
                 static_cast<float>(con));
      }

      for (unsigned int bin = 0; bin < wavlenStruct.adcCounts_size(); bin++) {
        int con = wavlenStruct.getAdcCounts(bin);
        tWav->Fill(wavlenStruct.getTime() + (static_cast<float>(bin) + 0.5) * wavlenStruct.getInterval(),
                   static_cast<float>(con));
      }
  ...
  tEdep->Draw("Hist");
  c->SaveAs(filename + "_Edep.png");

  TF1* grE_DR = new TF1("Efit", "gaus", low, high);
  grE_DR->SetLineColor(kBlack);
  tE_DR->SetOption("p");
  tE_DR->Fit(grE_DR, "R+&same");
  ...
  statsE_DR->SetY1NDC(.7);
  statsE_DR->SetY2NDC(1.);

  tE_S->SetTitle("");
  tE_S->Draw("Hist&sames");
  c->Update();
  ...
  TF1* grE_SC = new TF1("S+Cfit", "gaus", 2. * low, 2. * high);
  grE_SC->SetLineColor(kBlack);
  tE_SC->SetOption("p");
  tE_SC->Fit(grE_SC, "R+&same");

  tE_SC->Draw("");
  c->SaveAs(filename + "_Esum.png");

  TGraph* grSvsC = new TGraph(E_Ss.size(), &(E_Ss[0]), &(E_Cs[0]));
  grSvsC->SetTitle("SvsC;E_S;E_C");
  grSvsC->SetMarkerSize(0.5);
  grSvsC->SetMarkerStyle(20);
  ...
  tT_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_tC.png");
  tT_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_tS.png");
  tWav_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_wavC.png");
  tWav_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_wavS.png");
  tNhit_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_nhitC.png");
  tNhit_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_nhitS.png");

  tInt_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_intC.png");
  tInt_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_intS.png");

  tD_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_dC.png");
  tD_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_dS.png");

  tToa_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_toaC.png");
  tToa_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_toaS.png");

  tProc_dual->getHist(1)->Draw("Hist");
  c->SaveAs(filename + "_procC.png");
  tProc_dual->getHist(0)->Draw("Hist");
  c->SaveAs(filename + "_procS.png");
