...
WGR16CellParameterisation::WGR16CellParameterisation(const G4int numx, const G4int numy)
...
void WGR16CellParameterisation::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const {
  physVol->SetTranslation(G4ThreeVector(fXCell[copyNo],fYCell[copyNo],0.));
}

bool WGR16CellParameterisation::IsCerenkov(G4int col, G4int row) const {
  bool isCeren = false;
